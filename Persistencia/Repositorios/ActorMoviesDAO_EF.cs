...
namespace Persistencia.Repositorios
{
...

    {
        ...
        public async Task Inserir(ActorMovie actorMovie)
        {
            await _context.ActorMovies.AddAsync(actorMovie);
            await _context.SaveChangesAsync();

        }
        public async Task<List<ActorMovie>> List()
        {
            return await _context.ActorMovies.ToListAsync();
        }

        public async Task Delete(ActorMovie actorMovie)
        {
        ...
        public async Task<ActorMovie> GetActorMovieById(int Id)
        {
            return await _context.ActorMovies.FindAsync(Id);

        }

        public async Task Update(ActorMovie actorMovie)
        {
            _context.ActorMovies.Update(actorMovie);
            await _context.SaveChangesAsync();

        }

        /*
        public async Task<ICollection<ActorMoviesummary>> getActorMoviesummary()
        {
            var query = from f in _context.Movies
                        group f by f.Genre.Name into grpGen
                        select new ActorMoviesummary
                        {
